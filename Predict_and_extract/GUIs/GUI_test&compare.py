...
def predict_in_batches(model, dataset, total_samples: int, batch_size: int, progress_callback=None) -> Tuple[np.ndarray, np.ndarray]:
    """Efficiently predicts in batches, ensuring correct sizes."""
    ...
    for i in range(steps):
        ...
        try:
            ...
            predictions.append(model.predict(batch_x, verbose=0))
...
class ModelAnalysisGUI:
    ...
    def analysis_task(self):
        ...
        try:
            ...
            test_ds = create_data_generator(self.test_dir, batch_size, model_categorical=self.model_categorical, model_binary=self.model_binary)
            ...
            self.current_predictions, self.current_true_labels, self.current_filenames = raw_predictions, true_labels, test_ds.filenames[:len(raw_predictions)]

            self.save_cache(self.get_cache_key(), predictions_data)
            self.root.after(0, self.update_results)
            self.progress_queue.put({'percent': 100, 'status': "Analysis finished!", 'time_remaining': ""})
...
class ModelComparisonGUI:
    ...
    def setup_gui(self):
        ...
        comparison_frame.grid(row=1, column=0, sticky="ew", pady=5)
        ...
        visualization_frame.grid(row=2, column=0, sticky="nsew", pady=5)
    ...
    def add_current_model(self):
        ...
        model_data = {'name': model_name, 'path': self.parent_app.model_path, 'predictions': self.parent_app.current_predictions, 'true_labels': self.parent_app.current_true_labels, 'filenames': self.parent_app.current_filenames}
        ...
        messagebox.showinfo("Success", f"Added model '{model_name}' {msg}.")
    ...
    def compare_models(self):
        if not self.models_data:
            messagebox.showinfo("Info", "Add models to compare.")
            return

        missing_preds_models = [m['name'] for m in self.models_data if m['predictions'] is None]
        if missing_preds_models:
            if messagebox.askyesno("Missing Predictions", f"Models missing predictions: {', '.join(missing_preds_models)}. Load from cache?"):
                self.load_predictions_from_cache()
            else: return

        missing_preds_models = [m['name'] for m in self.models_data if m['predictions'] is None] # Re-check after cache load
        if missing_preds_models:
            if messagebox.askyesno("Missing Predictions", f"Still missing predictions for: {', '.join(missing_preds_models)}. Run predictions in main app first."):
                pass # User needs to run in main app
            return
    ...
    def load_predictions_from_cache(self):
        ...
        for i, model_data in enumerate(self.models_data):
            if model_data['predictions'] is not None: continue
            cache_key = f"{model_data['path']}_{test_dir}"
            if cache_key in cache:
                cached_data = cache[cache_key]['predictions_data']
                self.models_data[i].update({
                    'predictions': cached_data['raw_predictions'],
                    'true_labels': cached_data['true_labels'],
                    'filenames': cached_data['filenames']
                })
                print(f"Loaded cache for '{model_data['name']}'")
